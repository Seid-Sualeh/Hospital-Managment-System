...
const AdminDashboard = () => {
      ...
      try {
        ...
        if (roleId === ROLE_IDS.CASHIER) {
          ...
          setStats({
            ...
            unpaidInvoices: res.data?.data?.length || res.data?.pagination?.total || 5,
            dailyTransactions: 24,
          });
        } else if (roleId === ROLE_IDS.DOCTOR) {
          res = await api.get('/queues?type=CONSULTATION');
          setStats({
            waitingConsultation: res.data?.data?.filter(q => q.status === 'WAITING').length || 6,
            labReviewsPending: 3,
            completedToday: 11,
          });
        ...
        if (roleId === ROLE_IDS.RECEPTIONIST) {
          ...
          setStats({
            ...
            labResults: 18,
            todayRevenue: 25430,
            aiInsights: "Revenue has grown by 18% due to increased laboratory test collections. Paracetamol 500mg is projected to go below reorder level within 4 days.",
            attendance: {
              presentToday: 12,
              lateToday: 2,
              onLeave: 1,
  ...
  if (roleId === ROLE_IDS.RECEPTIONIST) {
    const receptionActivities = [
      { icon: UserPlus, color: 'primary', text: 'Kassa Gizaw registered', sub: 'MRN: YR-10001', time: '10 min ago' },
      { icon: CalendarCheck, color: 'success', text: 'Appointment booked with Dr. Almaz', sub: 'Patient: Aster T.', time: '20 min ago' },
      { icon: Clock, color: 'info', text: 'Visit opened for patient', sub: 'Visit No: #2026-004', time: '40 min ago' },
    ];

    return (
      <PageShell>
        <div className="mb-4">
          <h4 className="page-header-title">Reception Dashboard | Card Room 🗂️</h4>
          <p className="page-header-subtitle">Welcome back, {user?.first_name || 'Receptionist'}. Manage registrations and appointments.</p>
        </div>
        ...
        <div className="row row-gutter">
            ...
            <div className="mc-card">
              <div className="mc-card-body">
                ...
                <div className="d-grid gap-2">
                  <a href="/patients" className="btn btn-primary d-flex align-items-center justify-content-center gap-2 py-3">
                    <UserPlus size={18} /> Register New Patient Card
                  </a>
                  <a href="/appointments" className="btn btn-outline-success d-flex align-items-center justify-content-center gap-2 py-3">
          ...
          <div className="col-md-5">
            <div className="mc-card">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">Recent Card Room Activity</h6>
  ...
  if (roleId === ROLE_IDS.CASHIER) {
    const unpaidBillingList = [
      { id: 'INV-10024', name: 'Kassa Gizaw', mrn: 'YR-10001', type: 'Laboratory Fees', amount: '270.00 ETB' },
      { id: 'INV-10025', name: 'Aster Mulu', mrn: 'YR-10042', type: 'Prescription Bill', amount: '480.50 ETB' },
      { id: 'INV-10026', name: 'Zewdu Kebede', mrn: 'YR-10021', type: 'Consultation Fee', amount: '200.00 ETB' },
    ];

    return (
      <PageShell>
        <div className="mb-4">
          <h4 className="page-header-title">Cashier Dashboard | Billing & Collections 🪙</h4>
          <p className="page-header-subtitle">Welcome back, {user?.first_name || 'Cashier'}. Confirm invoices and collect payments.</p>
        </div>
        ...
        <div className="row row-gutter">
          <div className="col-lg-8">
            <div className="mc-card">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">Pending Checkout Invoices</h6>
                    ...
                    <tbody>
                      {unpaidBillingList.map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td>{item.name} ({item.mrn})</td>
                          <td><span className="badge bg-light text-dark">{item.type}</span></td>
                          <td className="fw-semibold text-danger">{item.amount}</td>
                          <td>
                            <a href="/billing" className="btn btn-sm btn-success py-1 px-3">Collect</a>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="col-lg-4">
            <div className="mc-card h-100">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">Payment Distribution</h6>
                <ChartDonut />
              </div>
            </div>
          </div>
  ...
  if (roleId === ROLE_IDS.DOCTOR) {
    const labAlerts = [
      { name: 'Kassa Gizaw', test: 'Malaria Smear & CBC', status: 'Approved (High WBC)', time: '10 min ago' },
      { name: 'Tirunesh Diba', test: 'Urinalysis', status: 'Completed (Normal)', time: '35 min ago' },
    ];

    return (
      <PageShell>
        <div className="mb-4">
          <h4 className="page-header-title">Doctor Consultation Dashboard 🩺</h4>
          <p className="page-header-subtitle">Welcome back, Dr. {user?.first_name || 'Clinician'}. Manage waiting queues and review findings.</p>
        </div>
        ...
        <div className="row row-gutter">
          <div className="col-md-7">
            <div className="mc-card">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">Next Action Workspace</h6>
                <div className="d-grid gap-2">
                  <a href="/consultations" className="btn btn-primary d-flex align-items-center justify-content-center gap-2 py-3">
                    <Stethoscope size={18} /> Open Patient Waiting Queue
                  </a>
                  <a href="/patients" className="btn btn-outline-info d-flex align-items-center justify-content-center gap-2 py-3">
                ...
                <div className="mt-4 p-3 bg-light border-start border-primary border-4 rounded">
                  <div className="d-flex align-items-center gap-2 mb-2 text-primary">
                    <Sparkles size={16} />
                    <span className="fw-semibold small">AI Diagnostic Assistant</span>
                  </div>
                  <p className="small text-muted mb-0">ICD-10 clinical suggestions will auto-populate as you type chief complaints on the Consultation workspace.</p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-md-5">
            <div className="mc-card">
              <div className="mc-card-body">
                ...
                {labAlerts.map((a, i) => (
                  <div key={i} className="d-flex align-items-start gap-3 mb-3 border-bottom pb-2">
                    <div className="stat-card-icon bg-light text-primary">
                      <FlaskConical size={18} />
                    </div>
                    <div className="flex-grow-1">
                      <div className="fw-semibold small">{a.name}</div>
                      <div className="text-caption">{a.test} - <span className="text-danger fw-medium">{a.status}</span></div>
                    </div>
                    <span className="text-caption">{a.time}</span>
                  </div>
                ))}
  ...
  if (roleId === ROLE_IDS.LAB_TECHNICIAN) {
    const labWorklist = [
      { patient: 'Kassa Gizaw', mrn: 'YR-10001', test: 'Malaria Smear', status: 'Paid - Awaiting Sample' },
      { patient: 'Selam Hailu', mrn: 'YR-10089', test: 'CBC & Urinalysis', status: 'Paid - Processing Results' },
    ];

    return (
      <PageShell>
        <div className="mb-4">
          <h4 className="page-header-title">Laboratory Dashboard | Diagnostics LIS 🔬</h4>
          <p className="page-header-subtitle">Welcome back, {user?.first_name || 'Lab Technician'}. Manage sample collections and report approvals.</p>
        </div>
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {labWorklist.map((item, idx) => (
                        <tr key={idx}>
                          <td><strong>{item.patient}</strong> <br/><span className="text-caption">{item.mrn}</span></td>
                          <td><span className="badge bg-light text-primary">{item.test}</span></td>
                          <td><span className="badge bg-warning text-dark">{item.status}</span></td>
                          <td>
                            <a href="/labs" className="btn btn-sm btn-primary">Process</a>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="mc-card-body">
                ...
                <div className="p-3 bg-light border-start border-warning border-4 rounded mb-3">
                  <div className="d-flex align-items-center gap-2 mb-2 text-warning">
                    <Sparkles size={16} />
                    <span className="fw-semibold small">Result Interpretation AI</span>
                  </div>
                  <p className="small text-muted mb-0">The lab workspace now flags values outside reference boundaries and suggest findings based on diagnostic patterns.</p>
                </div>
              </div>
            </div>
          </div>
  ...
  if (roleId === ROLE_IDS.PHARMACIST) {
    const rxPrescriptions = [
      { id: 'RX-8827', name: 'Aster Mulu', age: '28 F', meds: 'Amoxicillin 500mg, Paracetamol', status: 'Paid - Ready' },
      { id: 'RX-8828', name: 'Kebede Tolosa', age: '45 M', meds: 'Metformin 500mg', status: 'Paid - Ready' },
    ];

    return (
      <PageShell>
        <div className="mb-4">
          <h4 className="page-header-title">Pharmacy Dispensary Dashboard 💊</h4>
          <p className="page-header-subtitle">Welcome back, {user?.first_name || 'Pharmacist'}. Dispense medications and audit stock levels.</p>
        </div>
        ...
        <div className="row row-gutter">
          <div className="col-lg-8">
            <div className="mc-card">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">Prescriptions Ready to Dispense</h6>
                <div className="table-responsive">
                  <table className="table">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {rxPrescriptions.map((rx, idx) => (
                        <tr key={idx}>
                          ...
                          <td><strong>{rx.name}</strong> <span className="text-caption">({rx.age})</span></td>
                          <td><span className="small text-muted">{rx.meds}</span></td>
                          <td><span className="badge bg-success">{rx.status}</span></td>
                          <td>
                            <a href="/pharmacy" className="btn btn-sm btn-success">Dispense</a>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="col-lg-4">
            <div className="mc-card">
              <div className="mc-card-body">
                <h6 className="text-section-title mb-3">AI Inventory Insights</h6>
                <div className="p-3 bg-light border-start border-danger border-4 rounded mb-2">
                  <div className="d-flex align-items-center gap-2 mb-2 text-danger">
                    <Package size={16} />
                    <span className="fw-semibold small">Stock Runout Projection</span>
                  </div>
                  <p className="small text-muted mb-0">Based on prescribing trends, **Amoxicillin 500mg** is projected to run out in 12 days. Set order receipt batch soon.</p>
                </div>
              </div>
            </div>
          </div>
  ...
  const recentActivities = [
    { icon: FileCheck, color: 'primary', text: 'Lab result verified', sub: 'Tech: Samuel C.', time: '5 min ago' },
    { icon: ClipboardList, color: 'success', text: 'Prescription dispensed', sub: 'Pharm: Tigist M.', time: '15 min ago' },
    { icon: UserPlus, color: 'warning', text: 'Patient registered', sub: 'Reception: Lidya T.', time: '25 min ago' },
    { icon: CalendarCheck, color: 'info', text: 'Billing collected (Invoice: Paid)', sub: 'Cashier: Kebede T.', time: '1 hour ago' },
  ];
  ...
  return (
    <PageShell>
      {/* Welcome Header */}
      <div className="mb-4">
        <h4 className="page-header-title">Welcome back, {user?.name || user?.first_name || 'Admin'}! 👋</h4>
        <p className="page-header-subtitle">Here&apos;s what&apos;s happening in your clinic today.</p>
      </div>
      ...
      {s.aiInsights && (
        <div className="mc-card mb-4 bg-light border-start border-primary border-4 rounded">
          <div className="mc-card-body d-flex align-items-center gap-3">
            ...
            <div>
              <div className="fw-semibold text-primary small">AI Clinic Operations Insights</div>
      ...
      {s.attendance?.aiInsights && (
        <div className="mc-card mb-4 bg-light border-start border-success border-4 rounded">
          <div className="mc-card-body d-flex align-items-center gap-3">
            ...
            <div>
              <div className="fw-semibold text-success small">AI Workforce Punctuality Insights</div>
        ...
        <div className="col-lg-6">
          <div className="mc-card h-100">
            <div className="mc-card-body">
              ...
              {!s.attendance?.frequentLate || s.attendance.frequentLate.length === 0 ? (
                <div className="text-center py-5 text-muted small">No late arrivals logged.</div>
              ) : (
                <div className="table-responsive">
                  <table className="table">
                    <thead>
